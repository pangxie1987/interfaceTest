...
from comm.config import email_conf, log_conf
...
def send_email(report_file, result):
    ...
    logfile = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'report',log_conf.filename)
    att2 = MIMEText(open(logfile, 'rb').read(), 'base64', 'utf-8')
    att2["Content-Type"] = 'application/octet-stream'
    att2["Content-Disposition"] = 'attachment; filename=%s'%(log_conf.filename)
    msg.attach(att2)
    try:
